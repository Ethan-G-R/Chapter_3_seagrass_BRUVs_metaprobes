...
process_blast_summary <- function(file_path) {
  ...
  blast_data <- blast_data %>%
    ...
    mutate(species = str_c(species, "_MiFishU_gen"))
...
Mi_tax_tab <- Mi_tax_tab %>%
  ...
  mutate(species = str_replace(str_remove(species, "_MiFishU_gen$"), "_", " "))
...
save(Met_Mi, file = "working_phyloseq_data/Met_Mi_gen_99_97.RData")
